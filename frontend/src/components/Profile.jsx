...
export default function Profile() {
  const { user, updateUser, refreshMe } = useAuth(); // đảm bảo AuthContext có các hàm này
  ...
  const handleAvatarSubmit = async (event) => {
    ...
    try {
      ...
      const busted = avatarUrl ? `${avatarUrl}?t=${Date.now()}` : avatarUrl;
      updateUser?.({ avatarUrl: busted });
      await refreshMe?.();
      setAvatarFile(null);
      if (avatarUrl) setAvatarMessage('Cập nhật avatar thành công.');
    } catch (err) {
  ...
  return (
    <div className="profile-page">
      ...
      <section className="profile-avatar-section">
        ...
        {avatarMessage && <p style={{ color: 'green' }}>{avatarMessage}</p>}
